# Anibal Arias

Software Development Manager focused on technical leadership, software delivery, process improvement, and business-oriented technology solutions.

My work connects business needs with technical execution: aligning teams, improving delivery practices, understanding architecture, and promoting sustainable software solutions.

## Areas of focus

- Technical team leadership
- Software delivery and execution
- Process improvement
- Business-oriented solutions
- Architecture awareness
- Automation and AI-assisted workflows
- Documentation and engineering practices

## About this GitHub profile

This space is a curated professional portfolio focused on selected projects, technical notes, delivery practices, architecture documentation, and automation experiments.

The goal is not to showcase volume, but clarity: how software initiatives are structured, documented, improved, and connected to business value.

## Featured repositories

- **[Project AI Email Secretary](https://github.com/anibal-tech/project-ai-email-secretary)**  
  AI-assisted workflow for email classification, prioritization, summarization, and draft responses.

- **[Software Delivery Playbook](https://github.com/anibal-tech/software-delivery-playbook)**  
  Notes and templates for branching strategies, pull requests, code reviews, releases, and Definition of Done.

- **[Architecture Notes](https://github.com/anibal-tech/architecture-notes)**  
  Lightweight documentation for technical decisions, API design, integration patterns, and system thinking.

 **[Business Automation Lab](https://github.com/anibal-tech/business-automation-lab)**  
  Practical automation experiments focused on reducing manual work, improving operational visibility, and supporting process improvement.

## Portfolio direction

This portfolio reflects my interest in connecting leadership, technology, and execution.

The repositories are organized around four complementary areas:

- AI-assisted business workflows
- Software delivery practices
- Architecture documentation
- Business process automation

Together, they represent how I approach technical work: with structure, documentation, business context, and continuous improvement.

## Connect

- [LinkedIn](https://www.linkedin.com/in/anibal-arias)
- [GitHub](https://github.com/anibal-tech)
- [Professional website](https://estha.com.mx)
- [Tambor Mayor](https://tambormayor.com)
